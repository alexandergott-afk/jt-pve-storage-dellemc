...
本外掛的任何一部分都還沒有在真正的 Dell EMC 陣列上跑過。所有面向陣列的行為都是依據 Dell 公開文件寫出來的；文件讀不到的部分則是推測 —— 而光是 PowerVault，這些推測就已經被證實錯了四次，每一次都是回頭去讀指南才發現的。
...
- 一台你敢重開機的節點；
- 一組具備所需權限的陣列帳號（PowerStore 需要 Storage Operator，PowerVault 需要 `manage`）；
- 陣列的管理位址，以及開在旁邊的陣列 GUI 或 CLI；
- 30 分鐘。
...
請把陣列的 GUI 開著。storage id 要**短** —— PowerVault 只給它 10 個字元、PowerFlex 9 個，太長會在建立時就被拒絕，而不是默默截斷。
...
**預期**：指令正常返回，而且陣列上出現一個名為 `pve-<cluster>-<node>` 的 host 物件，帶著這台節點的 IQN 或 WWPN。

| 你看到的 | 代表什麼 |
|---|---|
| `Cannot reach the array at ...` | 管理位址、帳密或 TLS 有問題。訊息中 `Array error:` 之後的是陣列自己的說法 |
| host 物件沒有出現 | 建立 host 的指令被拒絕了；請看陣列的事件記錄裡它說了什麼 |
| 這台節點的 IQN 掛在**別的** host 物件上 | 先前的手動註冊佔用了它。請在陣列上移除，或設定 `dell-cluster-name` 讓產生的名稱與既有的一致 |
...
**預期**：儲存是 `active`，容量與陣列自己的 UI 相差在 1% 以內，而整個指令只花幾秒。如果要花上數十秒，請當成問題回報 —— 慢的儲存會餓死鄰居，而那正是 `dell-status-timeout` 要限制的事。
...
# 2. 由陣列 WWN 推導出的 WWID，與主機實際看到的比對
/lib/udev/scsi_id -g -u /dev/sdX

# 3. 陣列公布了哪些 iSCSI portal，以及這台節點實際連上了哪些
iscsiadm -m session
...
**預期**：陣列上出現名為 `pve-ps1-999-disk0`（PowerStore）或 `pve-ps1-999-d0`（PowerVault、PowerFlex）的 volume，已對應到這台節點，而 `/dev/mapper` 底下出現對應裝置。
...
| volume 建好之後又被刪掉 | 對應失敗，外掛把它回滾了。陣列的拒絕理由就在訊息裡 |
...
**預期**：陣列上出現名為 `<volume>.pve-snap-before`（PowerStore）或 `<volume>-s-before`（PowerVault、PowerFlex）的快照，而還原正常返回。

「還原到不是最新的那個快照」是**刻意**被拒絕的：Dell 沒有說明還原之後，比還原點更新的快照會怎麼樣，而本外掛不願意讓 PVE 繼續列出陣列可能已經清掉的還原點。若你確認了自己陣列的行為，`dell-rollback-any-snapshot 1` 可以解除這個限制 —— 也請告訴我們你的發現。
...
第一步必須失敗，而且訊息中要指出是哪個相依的複製擋住了。如果它成功了，代表這台陣列允許刪除仍有相依物件的範本，這值得回報。
...
**預期**：陣列上什麼都不剩、沒有 multipath map、也沒有殘留的 `sd` 路徑。殘留的 `sd` 裝置平時是安靜的，直到下一次 `multipathd` reload 用 `EBUSY` 灌滿 journal；詳見 [TROUBLESHOOTING_zh-TW.md](TROUBLESHOOTING_zh-TW.md)。
...
同一時段陣列自己的事件記錄，和上面任何一項一樣重要：本外掛回報的是陣列告訴它的話，而陣列通常說得更多。
