...
- **Unity：陣列的錯誤編號從未送到操作者眼前。** `error_code_of` 在 0.7.70
  加入、有測試、卻沒有任何呼叫者 —— 正是教訓 36 的形狀，靠 grep 呼叫端找到。
...
- **PowerFlex：死掉的陣列要付兩次「死掉」的代價。** 0.7.75 的多位址容錯為
  ...
  **世代**（先 4.x、後 3.x）形狀一模一樣，卻沒有拿到同一個上限。在有兩個位址的
...
### 修正
- **Unity：建立 LUN 時的儲存池鍵是錯的 —— 又是教訓 28 的形狀。** Dell 的
...
### 變更
- 錯誤內容的形狀是 `{error: {errorCode, httpStatusCode, messages}}`，而
...
- **Unity XT，第四個 storage type：`dellunity`。** 一位客戶有一台走 Fibre
  ...
  **這裡沒有任何一項在 Unity 陣列上執行過** —— 但其中極少是用猜的。每一個 URI、
  ...
  的 LUN、仍有複製在讀取的快照、仍在對應中的 LUN、超過 63 個字元的名稱。這個形狀
