...
- **Proxmox VE 自己建構的四種磁碟區名稱，我們有兩種認不得。** 大多數名稱是由
  ...
  **真正要緊的是 fleecing。** `vzdump --fleecing` 會在 fleecing 儲存上為每顆磁碟配置
  ...
  看起來就像一顆沒人交代得出來的磁碟。現在兩種名稱都能來回轉換：陣列上的物件帶著 PVE
...
- **文件站改為先講操作者判斷「該不該信任它」所需要的東西**，形式參考相關專案
  ...
  - **支援的 Proxmox VE 操作** —— 以操作本身為列的表格，虛擬機與容器分欄，並多一欄
    ...
    陣列跑過。那就是一個 beta 外掛誠實的狀態，它該放在功能列表之上，而不是之下。
  - **為什麼這個外掛需要實機驗證** —— Dell 四個系列都有公開文件，而那並不足夠；這裡
    每一項面向陣列的認知都與 Dell 自己的客戶端程式碼交叉比對過，而在兩台真實陣列上的
    第一次執行，仍然找出了再怎麼讀文件都找不到的缺陷。
  - **如何協助這個專案** —— 它缺的不是程式，是在別人陣列上的時間，以及那三個只有陣列
    答得出來的問題。
...
- **擴充之後，其他每一台節點都還停在舊大小。** 只有執行該客體的節點會做 resize ——
  ...
  `activate_volume` 現在會比對陣列回報的大小與本機 map 的大小（這兩個數字它本來就有），
  在陣列那個比較大時重新整理。反過來的方向永遠不做。
...
- **遷移過去的 VM 可能落在一個看不到自己磁碟的節點上。** `activate_volume` 是遷移
  ...
  host 物件**的節點上（因為陣列上的 host 是別人建立的、屬於採用而非新建），那個名稱
  會是外掛產生的名字，而它在陣列上並不存在。對應因此以 *Host … is not registered on
...
- 為 0.7.98 新增的 host 物件行為補上文件，兩種語言都有：`docs/FIRST_RUN` 新增一節
  說明「陣列上原本就有屬於這台節點的 host」，`docs/TROUBLESHOOTING` 涵蓋兩種拒絕
...
### 新增
- **改為使用陣列上原本就有的、屬於本節點的 host 物件，而不是再建一個。** 在這個外掛
  跑起來之前，陣列上通常已經有一個 —— 由當初做 fabric 分區的人建立 —— 以它自己的
  ...
  對陣列的請求。若記錄指向陣列上已經不存在的 host，該記錄會被丟棄，外掛回到自己的
  命名。
...
- **「initiator 已被註冊」現在會說出是哪個 host 佔用它。** PowerStore 規定一個
  ...
  沒有指出是哪個 host。外掛現在會反過來問陣列：這個節點的每個埠分別在誰身上，然後
...
- **initiator 以身分比對，而不是以寫法比對。** 同一個埠，PowerStore 寫成
  ...
  只要寫法不同，就會去重複加入一個 host 已經有的 initiator —— 而陣列會拒絕，儲存
  因此變成 inactive。
...
- **被移除的儲存會永遠留下本機的簿記資料。** `on_delete_hook` 只刪了密碼，其餘不管，
  ...
  WWID 與臨時複製的追蹤則刻意不同：如果裡面還記著本節點上的裝置或陣列上的物件，檔案
...
- **災難復原工具忽略了 `dell-host-mode`。** 這個選項決定外掛是「每個節點一個 host
  ...
  —— 而在這些陣列上，一個 initiator 同時只能屬於一個 host 物件，所以這件事最好的
...
### 已修正
- **同一台陣列上的兩個儲存共用了一個 client，連密碼也一起共用。** API client 的
  快取鍵是「管理位址 + 帳號 + TLS 設定 + 健康旗標」—— 而被快取的物件身上帶著它在
  每一則訊息裡指名的 storeid，以及密碼(自 0.7.86 起是逐儲存從
  `/etc/pve/priv/storage/<storeid>.pw` 讀出來的)。同一台陣列上有兩個儲存、帳號相同
  ...
  而陣列的管理帳號一旦被鎖定，那台陣列上的每一個儲存都會跟著倒。現在四個家族的鍵
  裡都有 storeid。

- **`get_identity`把同一台陣列說成兩台。** 它回答的是「這兩個儲存是不是同一個儲存」，
  而它是拿 `dell-portal` 原樣去組的 —— 而自 0.7.75 起，對於每個控制器各有自己管理
  IP 的陣列，這個欄位是以逗號分隔的清單。兩個控制器對調順序、逗號後多一個空格、
  或是一個儲存列了兩個而另一個只列一個，答案都會變成「不同的陣列」。現在這些位址
...
- **PowerVault ME4024 跑完了首次執行測試之外的六項生命週期項目，六項全過。**
  韌體 `GT280R011-01`、走 Fibre Channel，測試版本 0.7.66：客體作業系統從陣列
...
- **啟動 VM 時，對磁碟做了一次沒有時間上限的 stat。**
  ...
  這個覆寫同時拒絕在 `path()` 於陣列問不到時回傳的預留路徑上啟動。把那個路徑交給
...
- **把磁碟移出這個儲存，先被宣告可用、然後被拒絕。**
  ...
  *0.7.95 更正：本條目原本也把 `qm move_disk` 移到其他儲存列在裡面，那是錯的。
  ...
  向核心讀取，而不是用猜的；以及，除非核心自己對裝置的識別與陣列指名的磁碟區一致，
  否則拒絕使用該裝置。最後這一項才是關鍵 —— 匯入會寫入整顆磁碟映像，而 `path()`
  在陣列連不上時有一個文件記載的預留路徑退路，把映像寫進那個路徑解析出來的東西，
  是這一版所能觸及最糟的後果。
