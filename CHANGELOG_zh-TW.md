...
- **把磁碟移出這個儲存,先被宣告可用、然後被拒絕。**
  ...
  沒有 path,而且結構上不可能有。於是 `qm move_disk` 移到 LVM 或 ZFS 儲存、
  `pvesm export`、`pvesm import` 與跨叢集移轉,全都在外掛剛宣告支援之後的下一個
  呼叫失敗,訊息裡指名的還是外掛自己剛宣告的那個格式。四個家族都是,從覆寫寫下
  的那天起。`LVMPlugin`、`RBDPlugin`、`ZFSPoolPlugin` 都是兩半一起實作,當初只
  讀了格式那一半。
