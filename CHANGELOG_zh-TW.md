...
- **不屬於任何 host group 的 host，原本仍有可能查不到。** 這是唯一還沒有人抓到
  的形狀：ME 的 CLI 會把 ungrouped host 印在另一個區塊，而它在 JSON 裡對應到
...
- **PowerVault：外掛找不到自己剛剛建立的 host。** 由一台 ME4024 回報。每一次
  ...
  由於下一版韌體仍可能以沒人看過的形狀回答，這裡另外加了第二道防線：把陣列
...
### 修正
- **`SHA256SUMS` 現在記錄的是實際提供的檔名。** GitHub 不會提供檔名含 `~` 的附件，會把它改寫成 `.`；因此以 Debian 檔名產生的 `SHA256SUMS`，列出的是一個下載後根本不存在的檔案 —— `sha256sum -c` 會回報 *「No such file or directory」*。而那正是照著安裝說明操作的人會遇到的情況，並且是靠**實際照做一次**發現的，不是靠閱讀說明。現在改名發生在計算雜湊之前，因此附件與校驗檔互相吻合。
